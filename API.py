...
def upload():
    ...
    for j in range(1,2):
        ...
        os.mkdir(dirname)
    box_extraction("0.jpg","./RUN/Cropped/OMR/")
    box_extraction("2.jpg","./RUN/Cropped/ANSWER_KEY/")
    box_extraction("1.jpg","./RUN/Cropped/ROLLNO_"+str(1)+"/")
    registration()
    list=components()
    print(list)
    cattempted=list[0]
    ...
    shutil.rmtree('RUN')
    return jsonify(attempted=str(int(cattempted)),unattemped=str(int(cunattempt)),wrong=str(int(wrong)),score=str(int(score)))
